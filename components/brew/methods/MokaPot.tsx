...
const RECIPES = getRecipesByMethod('moka-pot')

export function MokaPot() {
  const [selectedSlug, setSelectedSlug] = useState<string>(() =>
    getStored('cba_mokapot_recipe', RECIPES[0]?.slug ?? ''),
  )
  const [coffeeGrams, setCoffeeGrams] = useState<number>(() =>
    getStored('cba_mokapot_coffee', 18),
  )
  ...
  useEffect(() => {
    localStorage.setItem('cba_mokapot_recipe', selectedSlug)
  }, [selectedSlug])

  useEffect(() => {
    localStorage.setItem('cba_mokapot_coffee', JSON.stringify(coffeeGrams))
  }, [coffeeGrams])
  ...
  if (phase === 'setup') {
    return (
      <div className="space-y-6">
        <div>
          <h2 className="font-serif text-3xl font-bold text-brand-espresso">Moka Pot Guide</h2>
          <p className="text-brand-brown/70 mt-1">
            Stovetop espresso. Intense, concentrated, and deeply satisfying.
          </p>
        </div>
        ...
        <Card>
          <h3 className="font-serif text-xl font-bold text-brand-espresso mb-4">
            {RECIPES.length > 1 ? '2.' : '1.'} Your Dose
          </h3>
          ...
          <div className="mt-3 text-sm text-brand-brown/60 space-y-0.5">
            ...
            <div>Fill water to the valve — do not exceed it.</div>
          </div>
        </Card>
        ...
        <Card>
          ...
          <p className="text-sm text-brand-brown/70 mb-4">
            Use <strong>pre-boiled hot water</strong> in the bottom chamber to reduce metal taste and speed up extraction. Keep heat on medium-low.
          </p>
    ...
    return (
      <div className="space-y-6">
        <div>
          ...
          <p className="text-brand-brown/70 mt-1">Serve immediately while hot.</p>
        </div>
        <Card>
            ...
            <div className="flex-1 mr-4">
              ...
              {currentStep.waterGrams > 0 && (
                <div className="text-brand-brown/70 mt-1">
                  Water: <strong>{currentStep.waterGrams}g</strong>
                </div>
              )}
            </div>
